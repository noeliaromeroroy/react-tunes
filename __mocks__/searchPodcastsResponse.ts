export const mockedSearchPodcastsResponse = {
  resultCount: 2,
  results: [
    {
      wrapperType: 'track',
      kind: 'podcast',
      artistId: 121676617,
      collectionId: 205892240,
      trackId: 205892240,
      artistName: 'BBC Radio 5 Live',
      collectionName: 'Test Match Special',
      trackName: 'Test Match Special',
      collectionCensoredName: 'Test Match Special',
      trackCensoredName: 'Test Match Special',
      artistViewUrl: 'https://podcasts.apple.com/es/artist/bbc/121676617?uo=4',
      collectionViewUrl:
        'https://podcasts.apple.com/es/podcast/test-match-special/id205892240?uo=4',
      feedUrl: 'https://podcasts.files.bbci.co.uk/p02nrsl2.rss',
      trackViewUrl:
        'https://podcasts.apple.com/es/podcast/test-match-special/id205892240?uo=4',
      artworkUrl30:
        'https://is1-ssl.mzstatic.com/image/thumb/Podcasts126/v4/1b/7f/71/1b7f71fb-663c-73d2-d7aa-802a3897e305/mza_15833932256799444157.jpg/30x30bb.jpg',
      artworkUrl60:
        'https://is1-ssl.mzstatic.com/image/thumb/Podcasts126/v4/1b/7f/71/1b7f71fb-663c-73d2-d7aa-802a3897e305/mza_15833932256799444157.jpg/60x60bb.jpg',
      artworkUrl100:
        'https://is1-ssl.mzstatic.com/image/thumb/Podcasts126/v4/1b/7f/71/1b7f71fb-663c-73d2-d7aa-802a3897e305/mza_15833932256799444157.jpg/100x100bb.jpg',
      collectionPrice: 0.0,
      trackPrice: 0.0,
      collectionHdPrice: 0,
      releaseDate: '2023-08-05T07:48:00Z',
      collectionExplicitness: 'notExplicit',
      trackExplicitness: 'cleaned',
      trackCount: 305,
      trackTimeMillis: 2910,
      country: 'ESP',
      currency: 'EUR',
      primaryGenreName: 'Deportes',
      contentAdvisoryRating: 'Clean',
      artworkUrl600:
        'https://is1-ssl.mzstatic.com/image/thumb/Podcasts126/v4/1b/7f/71/1b7f71fb-663c-73d2-d7aa-802a3897e305/mza_15833932256799444157.jpg/600x600bb.jpg',
      genreIds: ['1545', '26'],
      genres: ['Deportes', 'Podcasts'],
    },
    {
      wrapperType: 'track',
      kind: 'podcast',
      collectionId: 1211148486,
      trackId: 1211148486,
      artistName: 'vaughanradio',
      collectionName: 'Test Your English',
      trackName: 'Test Your English',
      collectionCensoredName: 'Test Your English',
      trackCensoredName: 'Test Your English',
      collectionViewUrl:
        'https://podcasts.apple.com/es/podcast/test-your-english/id1211148486?uo=4',
      feedUrl:
        'https://www.ivoox.com/test-your-english_fg_f1309388_filtro_1.xml',
      trackViewUrl:
        'https://podcasts.apple.com/es/podcast/test-your-english/id1211148486?uo=4',
      artworkUrl30:
        'https://is2-ssl.mzstatic.com/image/thumb/Podcasts125/v4/88/bb/39/88bb3976-a296-f4f7-94a3-d7246d79ef54/mza_10496370084980525431.jpg/30x30bb.jpg',
      artworkUrl60:
        'https://is2-ssl.mzstatic.com/image/thumb/Podcasts125/v4/88/bb/39/88bb3976-a296-f4f7-94a3-d7246d79ef54/mza_10496370084980525431.jpg/60x60bb.jpg',
      artworkUrl100:
        'https://is2-ssl.mzstatic.com/image/thumb/Podcasts125/v4/88/bb/39/88bb3976-a296-f4f7-94a3-d7246d79ef54/mza_10496370084980525431.jpg/100x100bb.jpg',
      collectionPrice: 0.0,
      trackPrice: 0.0,
      collectionHdPrice: 0,
      releaseDate: '2023-07-20T11:29:00Z',
      collectionExplicitness: 'notExplicit',
      trackExplicitness: 'cleaned',
      trackCount: 20,
      trackTimeMillis: 3423,
      country: 'ESP',
      currency: 'EUR',
      primaryGenreName: 'Aprendizaje de idiomas',
      contentAdvisoryRating: 'Clean',
      artworkUrl600:
        'https://is2-ssl.mzstatic.com/image/thumb/Podcasts125/v4/88/bb/39/88bb3976-a296-f4f7-94a3-d7246d79ef54/mza_10496370084980525431.jpg/600x600bb.jpg',
      genreIds: ['1498', '26', '1304'],
      genres: ['Aprendizaje de idiomas', 'Podcasts', 'Educación'],
    },
  ],
};