...
import {
  mockedSearchPodcastsResponse,
  mockedGetPodcastDetailResponse,
} from '../__mocks__/';
...
describe('searchPodcasts', () => {
  ...
  it('should throw an error when there is a network failure', async () => {
    ...
    await expect(searchPodcasts('networkFailure')).rejects.toThrow(
      'Network failure',
    );
  });
  ...
  it('should throw an iTunes error when response is not ok', async () => {
    ...
    await expect(searchPodcasts('iTunes returns error')).rejects.toThrow(
      /iTunes Error:/,
    );
  });
});
